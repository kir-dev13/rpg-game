...
function addNewSprite(category, imgSrc, name, x, y) {
  x *= 64;
  y *= 64;
...
function createSpritesFromAtlas(category, imgSrc, array) {
  for (let i = 0; i < array.length; i += 1) {
    const [name, x, y] = array[i];
    addNewSprite(category, imgSrc, name, x, y);
  }
}
// addNewSprite('terrain', 'mySprite', terrainAtlas, 8, 4);
// prettier-ignore
  ...
  map.forEach((cfgRow, y) => {
    cfgRow.forEach((cfgCell, x) => {
      ...
      const [
        ...
        sW,
        sH,
        ] = sprites.terrain[cfgCell[0]].frames[0];
      // console.log('🚀 ~ file: index.js ~ line 51 ~ cfgRow.forEach ~ cfgCell[0]', cfgCell[0]);
